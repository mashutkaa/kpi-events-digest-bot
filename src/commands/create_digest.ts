...
export const createDigestCommandReply =
    (bot: Telegraf) => async (ctx: Context) => {
        ...
        const todayKyivISO = new Intl.DateTimeFormat("en-CA", {
            timeZone: "Europe/Kyiv",
            year: "numeric",
            month: "2-digit",
            day: "2-digit",
        }).format(new Date());
        const nowKyivText = new Intl.DateTimeFormat("uk-UA", {
            timeZone: "Europe/Kyiv",
            dateStyle: "full",
            timeStyle: "short",
        }).format(new Date());
        ...
        const eventsText = events
            .map((e, index) => {
                const source = e.sourceUrl ? e.sourceUrl : "(none)";
                return `POST ${index + 1}\nSOURCE: ${source}\nTEXT:\n${e.text}`;
            })
            .join("\n\n---\n\n");
...
Сьогоднішня дата для фільтрації минулих подій: ${todayKyivISO} (час: ${nowKyivText}).
ВАЖЛИВО: Якщо подія вже відбулася (дата/час події в минулому відносно сьогоднішньої дати) — НЕ додавай її до дайджесту.

ВАЖЛИВО ПРО ПОСИЛАННЯ: Кожен вхідний допис має рядок SOURCE з URL першоджерела (або (none)).
- Якщо для події є релевантний SOURCE URL, то заголовок оформлюй як клікабельний: <b><a href="URL">Назва події</a></b>
- Якщо SOURCE = (none), тоді заголовок без посилання: <b>Назва події</b>
...
📆 <b>Дайджест заходів на найближчий час</b>
